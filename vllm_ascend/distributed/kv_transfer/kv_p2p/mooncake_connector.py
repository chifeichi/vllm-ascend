...
class KVCacheRecvingThread(threading.Thread):
    ...
    def _handle_request(self, req_meta: dict[str, Any]):
        ...
        req_meta["handler_start_time"] = task_start_time
    ...
    def _record_transfer_timing(
        ...
        timing = self.request_transfer_timings.get(remote_request_id)
        if timing is None:
            timing = {
            ...
            self.request_transfer_timings[remote_request_id] = timing
        ...
        timing["max_queue_depth"] = max(
            ...
            req_meta.get("queue_depth_at_enqueue", 0),
        )
        ...
        if should_log or timing["failed"]:
            receiver_queue_wait_ms = (
                timing["first_task_start_time"] - timing["first_enqueue_time"]
            ) * 1000
            transfer_window_ms = (task_end_time - timing["first_enqueue_time"]) * 1000
        ...
        del self.request_transfer_timings[remote_request_id]
    ...
    def _transfer_kv_cache_all_groups(self, req_meta: dict[str, Any]):
        ...
        if ret < 0:
            print(
                ...
                f"slice_count={len(length_list)} "
                f"prepare_ms={req_meta['prepare_ms']:.3f} "
...
class MooncakeConnectorWorker:
    ...
    def _get_hybrid_remote_rank_group_pulls(
        ...
        for group_id, (group_spec, layer_indices) in self.kv_group2layeridx.items():
            ...
            chosen_rank_list = self._get_remote_ranks_for_req(req_id, prefill_tp_size)[decode_tp_rank]
    ...
    def _get_hybrid_remote_port_send_num(
        ...
        for remote_rank in range(num_prefill_workers):
            remote_host_info = meta.remote_multi_nodes_meta_mapping.get(str(remote_rank))
            remote_host = meta.remote_host if remote_host_info is None else remote_host_info["host"]
            remote_port_send_num[meta.remote_port + remote_rank] = {"num": 0, "host": remote_host}
